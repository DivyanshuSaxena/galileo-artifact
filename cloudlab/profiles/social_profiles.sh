...
# 4: Name for the log
# 5: Workload to use (compose_post/read_home/read_user/all)
...
for load in $LOADS; do
  ...
  ssh ${CLIENT_PORT_SSH} -o StrictHostKeyChecking=no ${CLIENT_HOST} "tmux new-session -d -s workload${load} \"
    pushd \$HOME/scripts/deployment/social &&
...
mkdir -p $7/social_rate${RATE}_${DIST}_${NAME}_${IF_STRESS}
pushd $7/social_rate${RATE}_${DIST}_${NAME}_${IF_STRESS}

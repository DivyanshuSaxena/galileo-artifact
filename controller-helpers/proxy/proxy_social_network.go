...
func RejectComposePost(r *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
	mapStats["compose_post"].logRequest()
	if limiterTable["compose_post"].Allow() == false {
		return r, goproxy.NewResponse(r, goproxy.ContentTypeText, http.StatusForbidden, "REJECT!")
	} else {
		return r, nil
	}
}
func RejectReadHome(r *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
	mapStats["read_home_timeline"].logRequest()
	if limiterTable["read_home_timeline"].Allow() == false {
		return r, goproxy.NewResponse(r, goproxy.ContentTypeText, http.StatusForbidden, "REJECT!")
	} else {
		return r, nil
	}
}
func RejectReadUser(r *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
	mapStats["read_user_timeline"].logRequest()
	if limiterTable["read_home_timeline"].Allow() == false {
		return r, goproxy.NewResponse(r, goproxy.ContentTypeText, http.StatusForbidden, "REJECT!")
	} else {
		return r, nil
...
var PostComposePostCondition goproxy.ReqConditionFunc = func(req *http.Request, ctx *goproxy.ProxyCtx) bool {
	return req.URL.Host == global_config.FrontendUrl &&
		req.Method == "POST" &&
		strings.Contains(req.URL.Path, "/post/compose")
}
var GetReadHomeTimelineCondition goproxy.ReqConditionFunc = func(req *http.Request, ctx *goproxy.ProxyCtx) bool {
	return req.URL.Host == global_config.FrontendUrl &&
		req.Method == "GET" &&
		strings.Contains(req.URL.Path, "/home-timeline/read")
}
var GetReadUserTimelineCondition goproxy.ReqConditionFunc = func(req *http.Request, ctx *goproxy.ProxyCtx) bool {
	return req.URL.Host == global_config.FrontendUrl &&
		req.Method == "GET" &&
		strings.Contains(req.URL.Path, "/user-timeline/read")
}
...
func main() {
	...
	proxy.OnRequest(PostComposePostCondition).DoFunc(RejectComposePost)
	proxy.OnRequest(GetReadHomeTimelineCondition).DoFunc(RejectReadHome)
	proxy.OnRequest(GetReadUserTimelineCondition).DoFunc(RejectReadUser)

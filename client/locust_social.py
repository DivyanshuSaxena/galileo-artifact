import random
import socket
from locust import HttpUser, LoadTestShape, task, tag, between, events
import base64
import os
...
image_dir  = Path(os.path.dirname(os.path.realpath(__file__)) + '/base64_images')
image_data = {}
image_names = []
...
class SocialMediaUser(HttpUser):
    ...
    def compose_post(self):
        ...
        try:
            ...
            response_time = r.elapsed.total_seconds() * 1000 if hasattr(r, 'elapsed') else None
            self.log_request(response_time, {'type': 'compose_post'})
        except Exception as e:
            self.log_request(None, {'type': 'compose_post'}, exception=e)
    ...
    def read_home_timeline(self):
        ...
        try:
            r = self.client.get(url, params=args, name='read_home_timeline',
                context={'type': 'read_home_timeline', 'start': start, 'user_id': args['user_id']})
            response_time = r.elapsed.total_seconds() * 1000 if hasattr(r, 'elapsed') else None
            self.log_request(response_time, {'type': 'read_home_timeline'})
        except Exception as e:
            self.log_request(None, {'type': 'read_home_timeline'}, exception=e)
    ...
    def read_user_timeline(self):
        ...
        try:
            ...
            response_time = r.elapsed.total_seconds() * 1000 if hasattr(r, 'elapsed') else None
            self.log_request(response_time, {'type': 'read_user_timeline'})
        except Exception as e:
            self.log_request(None, {'type': 'read_user_timeline'}, exception=e)

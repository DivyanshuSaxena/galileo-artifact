...
class Agent:
    def __init__(self, target_apis, algo, interval=2, alpha=0.9, ttl=15000, code='online_boutique'):
        ...
        self.detector = Detector(use_shield=use_shield)
...
parser = argparse.ArgumentParser(description="Deploy RL agent")
parser.add_argument("--checkpoint", type=str, default=os.path.expanduser(global_config["checkpoint_path"]),
                    help="Path to RL checkpoint")
parser.add_argument("--use_shield", action="store_true", help="Whether to use shield or not")
...
detector = Detector(use_shield=use_shield)
current_agent = []
